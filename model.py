...
class Food(db.Model):
    ...
    def __repr__(self):
        return f'< Recipe id = {self.id}, dish name = {self.dish_name}, description = {self.description}, ingredients = {self.ingredients}, instruction = {self.instruction}, prep time = {self.prep_time}, total time = {self.total_time}, image = {self.image_url} >'
...
class Wine(db.Model):
    ...
    user = db.Column(db.Integer, db.ForeignKey(
        "users.id"))
    food = db.Column(db.Integer, db.ForeignKey('foodrecipes.id'))
